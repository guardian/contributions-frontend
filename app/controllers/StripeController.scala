...
import models._
import monitoring.{ContributionMetrics, LoggingTagsProvider, TagAwareLogger}
...
class StripeController(paymentServices: PaymentServices, stripeConfig: Config, ophanService: OphanService)(implicit ec: ExecutionContext)
  ...
  def pay = (NoCacheAction andThen MobileSupportAction andThen ABTestAction)
    .async(BodyParsers.jsonOrMultipart(ContributionRequest.contributionForm)) { implicit request =>
      ...
      logStripePaymentAttempt(platform)
      ...
      createCharge.map { charge =>
        ...
        Ok({
          info(s"Stripe payment successful for request id: ${request.id.toString}, \n" +
            s"\t from platform: $platform")
          logStripePaymentSuccess(platform)
        ...
        case e: Stripe.Error => {
          warn(s"Payment failed for request id ${request.id.toString}, \n" +
            s"\t from platform: $platform, \n" +
            s"\t with code: ${e.code}, \n" +
            s"\t decline code: ${e.decline_code}, \n" +
            s"\t and error message: ${e.message}.")
          logStripePaymentFailure(platform, e.code)
    ...
    NoCacheAction.async(parse.json) { implicit request =>
      ...
      withParsedStripeHook(request.body) { stripeHook =>
        ...
        stripeService.processPaymentHook(stripeHook)
          .value.map {
          case Right(_) =>
            logStripeHookSuccess()
            Ok
          case Left(_) =>
            logStripeHookFailure()
            InternalServerError
        }
      }
    }
